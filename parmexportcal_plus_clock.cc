//# Edited by Colm Coughlan 27.2.15
//# Dublin Institute of Advanced Studies
//# Should export clock solutions in the same way
//#
//#
//#
//#
...
#include "include/lofar_config.h"
...
#include "include/Package__Version.h"
...
void processReal (const Record& realValues, double perc,
                int skipLast, ParmDB& parmdb, parmValType type)
{
  for (uInt i=0; i<realValues.nfields(); ++i)
    ...
    int nf = real.shape()[0];
    const IPosition& shp = real.shape();
    // Get the median clock solution per frequency.
    // Optionally ignore the last value.
    int nt = std::max (1L, shp[1] - skipLast);
    Matrix<double> result(nf,1);
    for (int i=0; i<nf; ++i)
    {
	Matrix<double> aline = real(IPosition(2,i,0), IPosition(2,i,nt-1));
	double med = median(aline);
		if (abs(aline(i,nt-1) - med) > perc/100*med)
		{
			LOG_INFO_STR ("clock solution " << aline(i,nt-1) << " differs more than "<< perc << "% from median " << med);
    		}
   	 result(i,0) = med;
    }
    ...
    Matrix<double> medClock   (result(IPosition(2,0,0), IPosition(2,nf-1,0)));
    Vector<Double> freqs (reals.asArrayDouble("freqs"));
    Vector<Double> freqw (reals.asArrayDouble("freqwidths"));

    writeParm (medClock, name, parmdb, freqs, freqw);
    }
}
...
// Colm added changes here
  vector<double> clockrange = pdbIn.getRange ("Clock:*");
  // Usually real and imaginary are used. Try to read them.
  Record clockValues = pdbIn.getValuesGrid ("Clock:*",
                                           range[0], range[1],
                                           range[2], range[3]);
  processReal (clockValues, amplPerc, skipLast, pdbOut, type);
}
...
int main (int argc, char *argv[])
{
  ...
  try {
    // Get input arguments.
    InputParSet input;
    input.setVersion ("27.2.2015 Colm Coughlan Edition");
    cout<<endl<<endl<<"WARNING! This is not the normal version of parmexportcal."<<endl<<endl<<endl;
    cout<<"This version will also export the median clock solutions."<<endl;
    cout<<"This will only work if clock solutions are present in the table."<<endl<<endl<<endl;
